...
def train(validate=True):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (landmarks, depth_maps) in enumerate(train_loader):
            ...
            # Optionally save checkpoint after every batch
        ...
        if validate:
            ...
            with torch.no_grad():  # Disable gradient computation for validation
                for val_landmarks, val_depth_maps in val_loader:
                    val_landmarks = val_landmarks.to(device)
                    val_depth_maps = val_depth_maps.to(device)

                    # Forward pass for validation
                    val_outputs = model(val_landmarks)

                    # Ensure depth_maps have the correct shape for comparison
                    if val_depth_maps.ndim == 3:
                        val_depth_maps = val_depth_maps.unsqueeze(1)

                    # Calculate validation loss
                    val_loss += criterion(val_outputs, val_depth_maps).item() * val_landmarks.size(0)
